...
func explore(caves: Table[string, seq[string]], revisits: int, last: string, lastSeen: CountTable[string]): int =
  if last == "end": 1 else:
    var seen = lastSeen
...
func part1*(input: seq[string]): int = input.toMap.explore(0, "start", initCountTable[string]())

func part2*(input: seq[string]): int = input.toMap.explore(1, "start", initCountTable[string]())
